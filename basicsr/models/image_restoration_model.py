...
class ImageCleanModel(BaseModel):
    ...
    def optimize_parameters(self, current_iter):
        # [🔥 修改点 2] 自动关闭 MixUp 的逻辑
        if self.mixing_flag:
            mixup_off_iter = self.opt['train']['mixing_augs'].get('mixup_off_iters', -1)
            if mixup_off_iter > 0 and current_iter >= mixup_off_iter:
                self.mixing_flag = False
                logger = get_root_logger()
                logger.info(f'Mixup has been turned off at iter {current_iter}.')

        self.optimizer_g.zero_grad()

        with autocast(enabled=self.use_amp):
            # [🔥 修改点 3] 将 MixUp 移到这里执行，接受 mixing_flag 的动态控制
            if self.mixing_flag:
                self.gt, self.lq = self.mixing_augmentation(self.gt, self.lq)
        ...
        if self.opt['train']['use_grad_clip']:
            # 注意：如果 yaml 里配置了 max_norm，这里写死 0.01 可能会有点紧
            # 但既然原来的代码就是 0.01，为了保持一致性，我们先不动它
